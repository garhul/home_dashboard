...
class TimeSeries {
     ...
     *
     */
    addSample(value, timestamp = Math.ceil(Date.now() / 1000)) {
        if (isNaN(value))
        ...
        this.#subsets = this.#subsets.map((subset) => {
            ...
            subset.extras.median = (subset.extras.median === null) ?
                value :
                this.#computeMedian(subset.series.map(([k, v]) => Math.ceil(v)));
